...
var martinRobotController = require('../controllers/controller.martinRobot.js');
...
router.post('/run', async function (req, res) {
    let martinRobotInput = req.body.input;
    let martinRobotResult = await parseInput(martinRobotInput);
    
    const inputOutputObject={
        input: martinRobotInput,
        result: martinRobotResult
    }
    martinRobotController.addResult(inputOutputObject  ,function (err, result) {
        if (err) {
            console.log(err);
            return res.json({
...
router.get('/get_all', function (req, res) {
    martinRobotController.getAllResults(function (err, result) {
        if (err) {
            console.log(err);
...
function processCommands(commands) {
	for (var i = 0; i < commands.length; i++) {
		if (!doCommand(commands.charAt(i)))
			return false;
	}
